...
<<'Target-Age-Sex'>>=
# merge dihy with target
dihy.target = merge(dihy, target)

library(nnet)
age.sex.lr = multinom( data=dihy, days ~ AgeAtFirstClaim * Sex, family = "binomial")
summary(age.sex.lr)

# predicted response
predict.age.sex = predict(age.sex.lr, newdata=dihy.target, type="probs")
     
# find max days with max prob
pp = as.data.frame(predict.age.sex)
pp = pp[,!(names(pp) %in% "0")]
predict.days = as.data.frame(cbind(row.names(pp),
                                   apply(as.matrix(pp),1, function(x) {
                                         names(pp)[which(x==max(x))]}
)))
summary.days = table(predict.days$V2)
summary.days

preds = cbind(dihy.target, predict.days)

head(predict.age.sex)
@
