...
# >>>>>>> c2b66170e9e068c35d46eec5ea50d64cc395c94f

# expand claims to matrix with indicators for procdeures and conditions ...
claim.matrix <- model.matrix(Year ~ MemberID  + 
                                ProcedureGroup + 
                                PrimaryConditionGroup +
                                CharlsonIndex ,
                              claims[,]) 
claim.matrix = cbind(claim.matrix, claims$Year)
colnames(claim.matrix)[68] = "Year"

# summarise

# library(plyr)
# claims.m = as.data.frame(claim.matrix)
# rm( claims, claim.matrix)
# claims.matrix.sum = ddply(claims.m, .(MemberID, Year), numcolwise(sum))
# str(claims.matrix.sum)
